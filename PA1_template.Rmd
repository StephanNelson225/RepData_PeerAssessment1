...
```{r historgram1, fig.width=10, fig.path="figures/plot-"}
require(ggplot2)
...
```{r dailyactivitypattern, fig.width=10, fig.path="figures/plot-"}
...
```{r historgram2imputedvalues, fig.width=10, fig.path="figures/plot-"}
...
```{r weekdayweekend, fig.width=10, fig.path="figures/plot-"}
require(plyr)
...
ggplot(aes(x=as.integer(interval), y=MeanSteps, group = WeekId), data=c) + 
           geom_line(color = "steelblue", size = 0.8) + 
           labs(title = "Figure 4: \nTime Series Plot of the 5-minute Intervals Steps Registered \n Between September 1, 2012 and October 30, 2012", y= "Mean Steps per Interval") + 
           scale_x_continuous(name="Time of Day", breaks=c(1,144,288), labels=c("Midnight",
           "Noon", "Eleven\nFifty-Five PM")) +
